...
0.1 Added code from project 1 and implemented calculate_mean function - Julia Adamczyk
0.2 Implemented covariance function - Brendan Aguiar
0.3 Started working on Experiment 1b (resize function and making calculating mean and covariance more general)
0.4 Started ClassifyPercentages function - Brendan Aguiar
0.5 Added two functions 1. creating new set randomized from original set of given size 2. derive_results - provides results for each experiment
	(not permanent, only prints it to the terminal), cleaned up the code a little - Julia Adamczyk
0.6 - modified case3 and matrix determinant function to incorporate non-diagonal matrices - Julia Adamczyk
*/
...
void derive_results(vector <float> dataset[], vector <float> new_dataset[], float errorRates[]);
...
int main() {
	...
	while (again)
	{
		...
		case 4:
			...
			cout << "Classifying SetA of original size..." << endl;
		...
		case 6:
			...
			printErrorReport(errorRates1, "SetA");
			break;
		case 7:
			cout << "Experiment on 20 samples (0.01% original size): " << endl;
			create_set(setA, dataset1, 20);
			derive_results(setA, dataset1, errorRates1);
			break;
		case 8:
			cout << "Experiment on 200 samples (0.1% original size): " << endl;
			create_set(setA, dataset2, 200);
			derive_results(setA, dataset2, errorRates1);
			break;
		case 9:
			cout << "Experiment on 2,000 samples (1% original size): " << endl;
			create_set(setA, dataset3, 2000);
			derive_results(setA, dataset3, errorRates1);
			break;
		case 10:
			cout << "Experiment on 20,000 samples (10% original size): " << endl;
			create_set(setA, dataset4, 20000);
			derive_results(setA, dataset4, errorRates1);
			break;
		case 11:
			again = false;
			break;
...
*/
void printMenu() {
	cout << "Select from the following choices...\n1. Load SetA from file\n2. Print SetA to terminal\n";
	cout << "3. Calculate mean and covariance for SetA\n4. Classify SetA (using calculated variables)\n5. Classify(Euclidean) SetA \n6. Print Error Report SetA \n";
...
void shuffle(vector<float> v, size_t m, size_t n) {
	for (m = 0; m < n - 1; m++)
	{
		...
		int j = m + rand() % (n - m);
		std::swap(v.at(m), v.at(j));
	}
}


void classifyPercentages(vector <float> class1[], vector <float> class2[], float r[][4]) {
	float mean1[4][2]; //[0][#] = .01%, [1][#] = .1%, [2][#] = 1%, [3][#] = 10%
	float mean2[4][2];
	float cov1[4][2][2] = { 0 };
	float cov2[4][2][2] = { 0 };;
	const int val[4] = { TOTAL_SIZE * .0001, TOTAL_SIZE * .001, TOTAL_SIZE * .01, TOTAL_SIZE * .10 };
	list <float> p[4][2]; //[0][#] = .01%, [1][#] = .1%, [2][#] = 1%, [3][#] = 10%

	for (int i = 0; i < 4; i++) //Loads percentage lists as well as ML estimation
	{
		shuffle(class1[0], 0, round(val[i] * .3)); //shuffles values of set before loading into new list
		shuffle(class1[1], 0, round(val[i] * .3));
		shuffle(class2[0], 0, round(val[i] * .7));
		shuffle(class2[1], 0, round(val[i] * .7));

		for (int j = 1; j <= round(val[i] * .3); j++)
		{
			p[i][0].push_back(class1[0].at(j));
			p[i][1].push_back(class1[1].at(j));
		}
		for (int j = 1; j <= round(val[i] * .7); j++)
		{
			p[i][0].push_back(class2[0].at(j));
			p[i][1].push_back(class2[1].at(j));
		}
		calculate_mean(p[i], mean1[i], mean2[i]);
		calculate_cov(p[i], mean1[i], mean2[i], cov1[i], cov2[i]);
		classify(p[i], mean1[i], cov1[i], mean2[i], cov2[i], r[i]);
	}
